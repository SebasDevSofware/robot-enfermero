  ...
  #define MAX_HR_DELTA 25            // Máximo salto de BPM entre lecturas
  #define MAX_SPO2_DELTA 8           // Máximo salto de SpO2 entre lecturas
  #define LECTURAS_ESTABILIZACION 15 // Paquetes válidos antes de confiar
  ...
  #define TIEMPO_RECORDATORIO_MS 60000
  ...
  void TaskPillReminder(void *pvParameters);

  // ==================== TAREA RECORDATORIO PÍLDORAS ====================
  void TaskPillReminder(void *pvParameters) {
    // Esperar 10 segundos tras el arranque para no saturar el inicio
    vTaskDelay(pdMS_TO_TICKS(10000));
    
    while (true) {
      // Generar número aleatorio entre 1 y 7
      int audioIndex = random(1, 8);
      
      Serial.printf("[MP3] Recordatorio: Reproduciendo audio %04d.mp3\n", audioIndex);
      mp3Player.playMp3Folder(audioIndex); 
      
      // Esperar 30 segundos exactos para el próximo recordatorio
      vTaskDelay(pdMS_TO_TICKS(TIEMPO_RECORDATORIO_MS));
    }
  }
  ...
  void TaskProcessTelemetry(void *pvParameters) {
    ...
    while (true) {
      if (xQueueReceive(telemetryQueue, &packet, portMAX_DELAY) == pdTRUE) {
        if (packet != NULL) {
          bool datosLogicos = true;
          
          // Solo verificar si ya tenemos un dato previo válido
          if (lastValidData.timestamp != 0) {
            // 1. Verificar saltos fisiológicamente imposibles (Deltas excesivos)
            int32_t deltaHR = abs(packet->heartRate - lastValidData.heartRate);
            int32_t deltaSpO2 = abs(packet->spo2 - lastValidData.spo2);
            
            if (deltaHR > MAX_HR_DELTA || deltaSpO2 > MAX_SPO2_DELTA) {
              datosLogicos = false;
            }
            
            // 2. Verificar valores fuera de rangos biológicos extremos
            if (packet->heartRate < 30 || packet->heartRate > 220 || 
                packet->spo2 < 40 || packet->spo2 > 100) {
              datosLogicos = false;
            }
          }

          if (datosLogicos) {
            // Incrementar estabilización si los datos son coherentes
            if (contadorEstabilizacion < LECTURAS_ESTABILIZACION) {
              contadorEstabilizacion++;
            } else {
              sistemaEstabilizado = true;
            }
            
            memcpy(&lastValidData, packet, sizeof(TelemetryPacket));
            memcpy(&processedData, packet, sizeof(TelemetryPacket));
            mensaje_recibido = true;
            fallUrgent = (packet->fallDetected == 1);
          } else {
            // Si los datos son ilógicos, reiniciamos la confianza del sistema
  ...
  void sistemaExpertoIA() {
    ...
    // BLOQUEO DE SEGURIDAD: Esperar a que los sensores se estabilicen
    if (!sistemaEstabilizado) {
      estadoPaciente = ESTADO_ESTABILIZANDO;
      nivelRiesgoIA = 0;
      memset(diagnosticoIA, 0, sizeof(diagnosticoIA));
      memset(recomendacionIA, 0, sizeof(recomendacionIA));
      strcpy(diagnosticoIA, "ESTABILIZANDO SENSORES...");
      strcpy(recomendacionIA, "POR FAVOR, ESPERE.");
      motoresDetener();
      return;
    }
  ...
  void setup() {
    ...
    if (mp3Player.begin(mp3Serial)) {
      mp3Player.volume(25);
      Serial.println("[MP3] Módulo de voz listo.");
      xTaskCreatePinnedToCore(TaskPillReminder, "PillReminder", 4096, NULL, 1, NULL, 1);
    } else {
      Serial.println("[ERROR] No se pudo comunicar con el módulo MP3.");
    }
